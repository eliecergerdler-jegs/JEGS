...
export function WebDesignPage() {
  ...
  return <>
    <section className="relative overflow-hidden px-5 py-16 md:py-24"><div className="mx-auto grid max-w-7xl items-center gap-10 lg:grid-cols-[1.1fr_.9fr]"><div><p className="text-xs font-black uppercase tracking-[.28em] text-brand-green">{content.eyebrow}</p><h1 className="mt-4 max-w-4xl text-4xl font-black leading-tight tracking-tight text-brand-white md:text-6xl">{content.title}</h1><p className="mt-6 max-w-2xl text-lg leading-relaxed text-brand-muted">{content.subtitle}</p><div className="mt-8 flex flex-col gap-3 sm:flex-row"><a href="#quote" onClick={() => trackConversion("quote_cta_click", { service: "web", cta_location: "hero" })} className="inline-flex justify-center rounded-2xl bg-brand-green px-5 py-3 text-sm font-black text-brand-bg">{content.quote}</a><Link to="/portfolio" onClick={() => trackConversion("portfolio_click", { destination: "portfolio", cta_location: "web_hero" })} className="inline-flex items-center justify-center gap-2 rounded-2xl border border-white/15 bg-white/5 px-5 py-3 text-sm font-bold text-brand-white">{content.work} <ArrowRight size={16}/></Link></div></div><div className="rounded-[2rem] border border-white/10 bg-white/[0.035] p-7"><p className="text-sm font-bold text-brand-green">{content.trust}</p><div className="mt-6 grid gap-3">{content.points.map(item => <div key={item} className="flex items-center gap-3 rounded-2xl bg-white/5 p-4 text-brand-white"><CheckCircle2 className="text-brand-green" size={20}/>{item}</div>)}</div></div></div></section>
    ...
    <Section eyebrow={content.portfolioEyebrow} title={content.portfolioTitle} subtitle={content.portfolioSubtitle}><PortfolioGrid projects={portfolioProjects.slice(0, 3)} /><div className="mt-9 text-center"><Link to="/portfolio" className="text-sm font-black text-brand-green">{content.fullPortfolio} <ArrowRight className="inline" size={16}/></Link></div></Section>
